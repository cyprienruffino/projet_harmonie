 ...
 */
	public void ecrirePartition() throws IOException, InvalidMidiDataException {
		...
		for (int i = 0; i < 4; i++) {
			Track track = selectChant(sequence,i);
			int d;
			int h = 0;
			int n=-1;
	...
	private static Track selectNote(Track track,int note,int d,int h,int i) throws InvalidMidiDataException{
		switch (note) {
		case (3):
	...
	private static Track selectChant(Sequence sequence,int i) throws InvalidMidiDataException{
	...
	private static int selectNoteChant(Accord[] partition,int chant,int j){
		int n=-1;
			switch(chant){
			case(0):
